...
def load_test_or_valid(testfile, word2id, max_relation_len, max_Q_len, test_size):
    ...
    for line in train_file_triples:
        parts=line.strip().split('\t')
        triples=parts[:-1]
        length_per_example.append(len(triples))
        men_Qs=parts[-1].lower()
        #first load relations and entity scores
        r_word_ids=[]
        r_word_lens=[]        
#         entity_score=[]
        for triple in triples:
            relation=triple.strip().lower()
            r_words=relation.split()
            if len(r_words)>1:
                r_words=[relation]+r_words
        ...
        if line_co==test_size:
            break       
    result=(relations, relation_lengths, remainQ_word_ids, remainQ_word_lens)
    print 'load', line_co, 'test examples over'

    return result, length_per_example, word2id        
def load_train(trainfile, testfile, max_relation_len, max_Q_len, train_size, test_size, mark):
    #load char_vocab, word_vocab
#     char2id={}
    word2id={}
    ...
    remainQ_word_ids=[]
    remainQ_word_lens=[]
    length_per_example_train=[]
    line_co=0
    for line in train_file_triples:
        parts=line.strip().split('\t')
        triples=parts[:-1]
        length_per_example_train.append(len(triples))
        men_Qs=parts[-1].lower()
        #first load relations and entity scores
        r_word_ids=[]
        r_word_lens=[]        
#         entity_score=[]
    ...
    result_test, length_per_example_test, word2id  = load_test_or_valid(testfile, word2id, max_relation_len, max_Q_len, test_size)
...
#     write_word_emb.close()
#     print 'initialized word embs written over'
    return result, result_test, length_per_example_train, length_per_example_test, len(word2id)   
