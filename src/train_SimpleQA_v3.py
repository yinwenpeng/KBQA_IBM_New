...
#     test_remainQ_word_len=test_data[9]      #matrix, each three values for a remain question: 3
    ...
    train_sizes=[len(train_pos_entity_char),  len(train_relations), len(train_entity_char_lengths),\
...
#     length_after_wideConv=ishape[1]+filter_size[1]-1
    ...
    #simi_comb_weights=theano.shared(numpy.asarray(rng.uniform(low=-1.0, high=1.0, size=(1,4)), dtype=theano.config.floatX), borrow=True)

    params = [char_embeddings, embeddings, char_conv_W, char_conv_b, q_rel_conv_W, q_rel_conv_b]#, q_desH_conv_W, q_desH_conv_b]#, simi_comb_weights]
    ...
    def SimpleQ_matches_Triple(ent_char_ids_f,ent_lens_f,rel_word_ids_f,rel_word_lens_f,
                       men_char_ids_f, q_word_ids_f, men_lens_f, q_word_lens_f):#, ent_score):
...
#                     cosine(q_desT_pool.output_maxpooling, desT_conv_pool.output_maxpooling)
    ...
    L2_reg =debug_print((char_embeddings** 2).sum()+(embeddings** 2).sum()+(char_conv_W** 2).sum()+(q_rel_conv_W** 2).sum(), 'L2_reg')#+(layer1.W** 2).sum()++(embeddings**2).sum()
    diversify_reg= Diversify_Reg(char_conv_W_into_matrix)+Diversify_Reg(q_rel_conv_W_into_matrix)
    ...
    test_model = theano.function([ent_char_ids_M, ent_lens_M, men_char_ids_M, men_lens_M, rel_word_ids_M, rel_word_lens_M,
                                  q_word_ids_M, q_word_lens_M], [loss_simi, simi_list],on_unused_input='ignore')
#           givens={
